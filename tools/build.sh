...
if [[ "${DRAFTS_MODE}" -eq 1 ]]; then
  ...
  if [[ "$(uname -s)" =~ ^(MINGW|MSYS|CYGWIN) ]]; then
      ...
      original_dir=$(pwd)
      trap 'cd "$original_dir" && git restore draft-scenarios/assets draft-scenarios/latexmkrc' EXIT
...
# Regular language-based processing - now always using pdflua engine
if [[ ${LANGUAGE} != en ]]; then
