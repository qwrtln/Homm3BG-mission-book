...
if [[ "${DRAFTS_MODE}" -eq 1 ]]; then
  ...
  if ! latexmk -pdflua -file-line-error -halt-on-error -interaction=nonstopmode -shell-escape -recorder $ARGS drafts.tex; then
    echo -e "${ERROR}"
    exit 1
  fi
...
if ! latexmk -pdflua -file-line-error -halt-on-error -interaction=nonstopmode -shell-escape -recorder -jobname="${JOB}" $ARGS "main_${LANGUAGE}.tex"; then
  echo -e "${ERROR}"
  exit 1
fi
