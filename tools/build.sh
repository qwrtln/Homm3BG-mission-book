...
# Silence lualatex output if building a single scenario
SILENT=""
if [[ "${SCENARIO}" != "" ]]; then
  SILENT="-silent"
fi
...
rm -f "main_${LANGUAGE}.aux" && \
  latexmk -pdflua -shell-escape ${SILENT} "main_${LANGUAGE}.tex"

# Rename the file if building a single scenario
FILE="main_${LANGUAGE}.pdf"
if [[ "${SCENARIO}" != "" ]]; then
  FILE="${SCENARIO}.pdf"
  cp "main_${LANGUAGE}.pdf" "${FILE}"
fi
