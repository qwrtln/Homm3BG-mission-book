...
\textit{Evil Dragon Lords threaten the entire Enroth. Heroes, with their armies united, march against them in an epic final stand.}  % no-check
...
\begin{itemize}
  ...
  \item During the game, players assemble a shared army. Every player can field Units from the shared army during Combat (shared Units' \svg{map} abilities can be activated each Round by only one player).  % no-check
...
\begin{tikzpicture}[overlay]
  \centering
  \node at (14, 1) {\includegraphics[scale=0.3]{\maps/stand_united_3p.png}};
  \node at (14, -4) {\footnotesize{\textbf{\MakeUppercase{3-PLAYER SCENARIO}}}};
