...
\begin{itemize}
  \item Add Neutral \svgunit{bronze} Rogues to your Units.
  \item 6 \svg{gold}
...
In addition to the Cooperative Mode rules (see page 11, ``Cooperative Mode'' in the Core Mission Book), reference the attached \pagetarget{Cooperative Play Rules}{Cooperative Play Rules} specific to this campaign.
...
% \begin{itemize}
%   \item \textbf{Necropolis Armies:} At the moment in which either The Crest of Valor is obtained by Player 1 or 2, or if either Player enters Near Map tile N2 or N5, both Charging Heroes will appear on the map on S1 and F1.
%     Charging Heroes move after the Players do, and take shortest route to the Player that has Crest of Valor or the Angel Wings.
%     They ignore everything else, except they will take mines only if flagged by Rampart Player (but they will not backtrack to flag a mine).
%   \item Every time a Necropolis Charging Army is defeated, they re-appear on the Necropolis Town (S1).
%     If the Town is occupied, they re-appear on the Necropolis Settlement (F1).
%     They can move again on the next upcoming Resource Round.
% \end{itemize}
...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \node at (16.9, -3.6) {\large{{\textbf{\textcolor{darkcandyapplered}{the Seer}}}}};
    ...
    % Arrow from "Man in the Southwest" to just below F12
  \draw[->, line width=1.5pt, darkcandyapplered] ($(southwest) + (-0.2, 0.2)$) to[out=125, in=225] ($(f12) + (0, -1.5)$);

  % Arrow from "Feredor the Seer" to a point slightly above the text
  \draw[->, line width=1.5pt, darkcandyapplered] ($(feredor) + (0.5, 0.3)$) to[out=90, in=330] ($(feredor) + (0.9, 2.1)$);
\end{tikzpicture}
