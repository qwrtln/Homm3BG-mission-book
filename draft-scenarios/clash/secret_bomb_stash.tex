...
\begin{itemize}
  \item A player wins when they score 3 VPs. The scenario ends immediately after the third bomb is deposited by any single player.
...
\textbf{Scoring Victory Points (VP):}

Player gets 1 VP after winning a siege/entering enemy town while carrying the bomb.
\begin{itemize}
  \item After such event, the victorious hero gets moved back to their town, and the Grail token is put back on the grail space in the center (if any hero stands on that field when that happens they immediately become the bomb carrier).
  \item The defending player does not lose control over their town after losing the siege.
\end{itemize}

You can mark VP by collecting Faction Cubes of your opponent's color or a color that is not used by any player.
...
\begin{tikzpicture}[overlay]
  ...
  \node at (14, -4) {\includegraphics[scale=0.2]{\maps/secret-bomb-stash-3p.png}};
...
\begin{tikzpicture}[overlay]
  \centering
  \node at (4.5, -5) {\includegraphics[scale=0.25]{\maps/secret-bomb-stash-4p.png}};
  \node at (6, -10) {\footnotesize{\textbf{\MakeUppercase{4-PLAYER SCENARIO}}}};
  \node at (12.1, -16.5) {\includegraphics[scale=0.25]{\maps/secret-bomb-stash-6p.png}};
