...
\begin{itemize}
  ...
  \begin{itemize}
    \item \nth{1} - \svg{morale_negative} Token (-1 hand size for Necropolis)
    \item \nth{2} - no morale
    \item \nth{3} and higher - \svg{morale_positive} Token (+1 hand size for Necropolis)
  \end{itemize}
  \item If playing in Alliance Mode, add a Gold Unit for each extra team member to the Dragon Utopia fight (+1 when 2-people teams, +2 when 3-people teams).\end{itemize}
...
\hommtablemulticol[]{16}{
  \begin{tabularx}{0.95\linewidth}{X}
    ...
    \lightcell[3.8]{\svg{ongoing-table} A Unit with an Arcane Spike placed on it has its special ability negated \\ and suffers -1 \svg{defense_yellow} for every Arcane Spike (Cube) on it} \\
  \end{tabularx}
}
